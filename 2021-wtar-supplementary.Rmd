...
The `gridsearch()` function of the `lcmm` package was used to run 5000 departures from random initial values for each multi-class model, using the one-class model to generate those startiogn values from. The maximum number of iterations within the `hlme()` function was set at 1000.
