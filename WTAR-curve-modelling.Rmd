...
this analysis, we selected data from the `r group_n$n[group_n$group =='PD']` 
Parkinson's and `r group_n$n[group_n$group == 'Control']` Control
...
increasing from 1 to 5. The `gridsearch()` function of the `lcmm` package was 
used to run 5000 departures from random initial values for each multi-class 
model, using the one-class model from which to generate the starting values. The
maximum number of iterations within the `hlme()` function was set at 1000. 
...
optimal model was selected on the basis of it having the lowest BIC (Bayesian 
information criterion) value of the five candidates. When reporting values from 
the model, we formed the interval in brackets following the maximum likelihood 
...
dat_with_classes_plus_highlights = dat_with_classes %>%
  mutate(highlight = case_when(subject_id_num %in% c(63, 111) ~ TRUE,
                                                         TRUE ~ FALSE)) %>%
...
ggsave('images/wtar-figure-2.pdf', width = fig_2_width, height = fig_2_height + 5,
       units = 'cm')
...
above-average estimated IQ levels. [***TODO:*** Add possibility that by volunteering
