...
wordcount: 'Abstract: up to 250 (189), main body: up to 3700 (2117: 336+468+57+362+894)'
---
...
were, however, published in 2001, well before formal MDS guidelines on 
...
therefore unclear to what extent findings from that original sample are 
valid in the face of the significant cognitive deterioration that can occur in 
Parkinson's.
...
deviations below norms for PD-MCI and ≥2.0 standard deviations for PD dementia,[@dalrymple-alford2011_CharacterizingMildCognitive; @wood2016_DifferentPDMCICriteria]
...
classifications were not useful in a New Zealand context and all participants 
...
$lcmm$ package [@proust-lima2017estimation-of-e] (version `r packageVersion('lcmm')`), 
...
set at 1000. Parameters corresponding to the best log-likelihood were used as 
initial values for the final estimation of the 
parameters[@proust-lima2017estimation-of-e]. The optimal model was selected on
the basis of it having the lowest BIC value.
...
\@ref(fig:figure-2-trajectories)A). Initially, however, we modelled WTAR scores 
as a polynomial (cubic) function of age, to allow for clusters that might 
exhibit curvilinear changes. The optimal cubic model showed relatively flat 
trajectories, and we therefore shifted to modelling WTAR as a linear function of
age, fitting candidate models ranging from one to five latent trajectory 
...
of all participants) had an intercept of `r high_intcpt_str` `r high_intcpt_intrvl`, 
...
indicated a decline of `r high_slope_str` `r high_slope_intrvl` points per 
decade (effectively flat). The Typical trajectory, by contrast had an intercept of 
`r typical_intcpt_str` `r typical_intcpt_intrvl`, gently declining at a rate of 
`r typical_slope_str` `r typical_slope_intrvl` points per decade.
...
figure_2 = cowplot::plot_grid(figure_2_upper, figure_2_lower, nrow = 2, axis = 'l',
...
WTAR scores were stable over time in both controls and people with Parkinson's, 
despite many of the latter developing dementia over the course of the study 
(Figure \@ref(fig:figure-2-trajectories)B). Latent class analysis revealed that 
in our particular sample, people fell into one of two trajectory clusters, but 
these appeared to be largely driven by there being two clusters
of performance at baseline, rather than trajectories that differed longitudinally.
Those with very high initial WTAR scores maintained near-ceiling 
performance levels, even if they subsequently developed dementia. The remaining 
...
relatively stable, with an estimated decline of only `r typical_slope_str` points 
per decade.
...
(released in 1997), which has been superseded by the WAIS-IV (2008), and in turn, 
the WAIS-V is currently under development. The TOPF (Test of Premorbid 
Function) estimates WAIS-IV IQ and was designed as a successor to the WTAR, 
but has had limited uptake.[@bright2020_ComparisonMethodsEstimating] The 
...
distribution of estimated IQ values was well above the supposed population mean
of 100.
...
are not able to be created in languages that do not share English's 
peculiarities. Hence we recommend that in future revisions of MDS guidelines,
such tests of premorbid function no longer be advocated as one of the means of 
