---
title: "Wechsler Test of Adult Reading in Parkinson’s: stable premorbid measure despite cognitive decline"
author:
...
guidelines as one of two recommended tests for this purpose (along with the 
...
deterioration that often occurs in Parkinson's.
...
individuals to _a priori_ sub-groups.
...
increasing from 1 to 5. The _gridsearch()_ function of the _lcmm_ package was 
...
optimal model was selected on the basis of it converging and having the lowest 
BIC (Bayesian information criterion) value of the five candidates. When 
reporting values from the chosen model, we formed the interval in brackets 
...
\@ref(fig:figure-2-trajectories)A). This was evident even in participants whose 
overall cognitive performance declined substantially, with progression to 
dementia (Figure \@ref(fig:figure-2-trajectories)B). We modelled WTAR scores as 
a linear function of age, with repeated measures within individuals, fitting 
candidate models ranging from one to five latent trajectory classes. We selected
the two-class model on the basis of it having the lowest BIC value. Full details
on the modelling process are provided in the Supplementary Material.

Individuals are assigned to the class for which they have the highest posterior
...
two trajectories are depicted with the raw data superimposed in Figure
...
is not reliable, because if the disease process reduces current performance on 
that measure, then inferring poorer cognitive reserve would not be valid.
...
The WAIS-IV (2008) is now in widespread use, and the WAIS-V is currently under
development. The TOPF (Test of Premorbid Function) estimates WAIS-IV IQ and was 
designed as a successor to the WTAR, but has had limited
...
robust finding that cohort IQ scores have increased substantially over time (by
up to 1 standard deviation per generation). Our sample would (on average) have 
been born well after their age-matched WAIS-III normative sample group and that
might partly explain their above-average estimated IQ levels. Another 
possibility is that those who volunteer for research are unlikely to be
...
neuropsychological test z-score. Any test that was lower by more than 1.5 SD
was deemed impaired. Of their sample of 139 non-demented people with 
Parkinson's, the WTAR method classified many more of them as having PD-MCI (79%)
than did a norms-based Level II classification method (33%). Marras _et al._ 
interpreted this as evidence that most people with Parkinson's have undergone at
least some decline relative to their premorbid function. Conversely it might
support our contention that the WTAR simply over-estimates that premorbid level 
of function – just as in our sample, where the distribution of estimated IQ 
values was well above the supposed population mean of 100.

The WTAR (and similar tests such as the NART) are also limited in that they are 
inherently tied to peculiarities of written and spoken English. The orthography 
(writing system) of a language can be described as deep/opaque when it contains 
many irregularities in its mappings of graphemes to phonemes, or 
...
in the TOPF as relatively simple (28 out of 70), resulted in more errors than a 
word like 'plethora' (ranked 42 out of 70), likely reflecting different word 
frequencies between North American and New Zealand English. After the 
...
peculiarities of English. In future revisions of international MDS guidelines, 
we therefore believe that a test of premorbid function like the WTAR should no 
longer be recommended as one of the means of formally establishing cognitive 
decline at an individual level in Parkinson's.
