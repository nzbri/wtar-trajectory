...
abstract: Background. The Wechsler Test of Adult Reading (WTAR) has been
  ...
  disease. To fulfil either role, however, WTAR scores need to be shown to 
  ...
  scores in people with  Parkinson's as a function of age. Methods. From 253
  ...
  were stable within both Controls and Parkinson's participants, even for those
  who progressed to dementia. In both groups and regardless of cognitive status,
  scores were higher than expected from population norms. Conclusion. The WTAR 
  was stable in Parkinson's even when participants declined from normal 
  ...
  diagnosing cognitive impairment in individuals with Parkinson's.
...
current cognitive function in a given individual might reflect long-standing low
cognitive ability, rather than a recent disease-related deterioration. In 
practice, however, one seldom has reference to prior formal serial testing, and 
so a valid method of estimating premorbid function is appealing. The Wechsler 
Test of Adult Reading (WTAR)[@strauss2006_CompendiumNeuropsychologicalTests] was
...
least one cognitive domain. For PD dementia, significant impairments ($\geq$ 
2.0 standard deviations below normative data) in at least two cognitive domains
...
Table \@ref(tab:table-1). All participants were speakers of New Zealand English,
and were assessed against the US norms of the WTAR, corrected for age, sex, and
...
"Whites"). [***TODO:*** provide ethnicity breakdown of our sample.]
...
trajectories. The resulting trajectories were close to straight lines so we
simplified the models to be a linear function of age. The models were not
...
table_df %>% 
  ...
  rename(' '           = name,
         "Parkinson's" = PD) %>%
  apa_table(caption = "Demographics of the Parkinson's and Control groups, as at their last assessment.", align = 'lrr')
...
# TODO: make this figure more readable and usable.
# e.g. highlight a few representative individuals' trajectories.
# e.g. highlight a person with high initial WTAR who declines to dementia but 
# retains high WTAR scores. Then highlight the same person's z-score data,
...
they subsequently developed dementia (Figure
\@ref(fig:figure-2-trajectories)A). The remaining 
...
above-average estimated IQ levels. [***T*ODO:*** Add possibility that by volunteering
for research, our sample may be non-representative of the population, but (i) 
this should apply more to the controls than the PD group, which did not seem to 
be the case, and (ii) this would apply to most other research projects too. The
other neuropsychological test scores _do_ decrease below population norms with
cognitive decline. If the WTAR was used as a reference for those scores, rather
than a z-score of zero, we would expect to see infalted rates of MCI and PDD 
diagnosis.]
