...
\@ref(fig:figure-1)A). Initially, however,
we modelled WTAR scores as a polynomial (cubic) function of age, to allow for 
sub-clusters that might exhibit curvilinear changes. The optimal cubic 
model showed relatively flat trajectories, and we therefore shifted to modelling
...
(SD `r round(sd(individuals$class_probability), digits = 2)`), with a value of 0.7 
...
connecting lines). In each panel, current cognitive status is shown as cognitively normal 
...
(Figure \@ref(fig:figure-1)B). Latent class analysis revealed that 
