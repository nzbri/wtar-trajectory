...
if (RUN_NEW_LINEAR_MODELS) {
  ...
  for (class_num in 2:max_classes_linear) {
    ...
    linear_models[[class_num]] <- 
      lcmm::gridsearch(rep = 5000, maxiter = 100, minit = linear_models[[1]], 
                       cl = cl,
                       hlme(WTAR ~ age_std, 
                            random = ~1 + age_std, 
...
a single trajectory model (see Supplementary Material), somewhat intermediate 
between the two reported here, would also describe the data relatively well. The
key findings are that the trajectories are relatively flat, and do not separate 
between performance between the Parkinson's and Control participants. 
