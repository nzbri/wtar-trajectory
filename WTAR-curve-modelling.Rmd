---
title: "Wechsler Test of Adult Reading in Parkinson’s: a stable yet problematic measure of premorbid function "
author:
...
- name: Michael R. MacAskill, PhD
  affiliation: '1,2'
shorttitle: WTAR as premorbid measure in Parkinson's
output:
  papaja::apa6_pdf:
...
abstract: Background. The Wechsler Test of Adult Reading (WTAR) has been
  ...
  poorly-suited for assessing current cognitive impairment in individuals with 
  Parkinson's.
keywords: Parkinson disease, cognitive impairment, dementia, neuropsychology, 
  premorbid function
wordcount: 'Abstract: 200 (of 250), main body: 2608 (of 4000)'
editor_options: 
...
# Need this to get around some bug in not being able to find fonts. But although
# the document at least now knits without error, the fonts used in figures show
# glitches:
library(showtext)
showtext.auto()
font_add('Arial Narrow', '~/Library/Fonts/Arial Narrow.ttf')
...
The burden of dementia in Parkinson's is well-established. Also well-accepted is
the value in detecting the transitional stage of mild cognitive impairment in 
Parkinson's, in particular because this may provide a therapeutic window for 
future treatments, before the irreversible pathological processes associated 
with the dementia state have occurred. As a transitional state, the criteria are
not self-evident for distinguishing mild cognitive impairment both from the 
realm of normal cognitive functioning at one end, and dementia at the other. 
Consequently, to provide for consistent approaches, in 2012, the International
Parkinson and Movement Disorder Society published its Task Force guidelines for
the diagnosis of mild cognitive impairment in Parkinson's disease
(PD-MCI).[@litvan2012_DiagnosticCriteriaMild] 
...
ability), the Task Force guidelines propose that significant impairment may be 
determined in three ways. Firstly, current performance on a test can be shown to
be below appropriate norms (this may be established relative to standardised 
norms for that test, or relative to a locally-gathered control group). This 
approach has the disadvantage that the cause of poor current performance cannot 
be distinguished between that due to (a) a recent decline (such as might be 
caused by the onset of MCI or dementia), or (b) a long-standing poor level of 
cognitive ability. The distinction between the two is important if the goal is 
to detect the beginning of a decline due to eventual dementia. To unambiguously
detect recent decline in cognitive status therefore requires some way of 
demonstrating a decline in function within an individual over time. To achieve 
this, the guidelines suggest seeking evidence of either a decline in performance
on serial testing, or, in the absence of such prior testing, evidence of a 
decline from an individual's _estimated_ premorbid level of function. For 
pragmatic reasons, however, methods based upon current norms still tend to 
predominate in research
...
research setting, even serial testing testing remains ambiguous, as the first 
available test session may itself be contaminated by early disease-related
cognitive impairment, producing a falsely low baseline and thereby impeding the 
ability to detect cogntive decline.
...
proposed in the Task Force guidelines as one of two recommended [***TODO:*** 
check their exact phrasing.] tests for this purpose (along with the National 
Adult Reading Test, NART).[@strauss2006_CompendiumNeuropsychologicalTests] Both
tests are based on the ability to correctly pronounce phonetically-irregular 
English words, such as 'porpoise' or 'hyperbole'. This ability has been shown to
be preserved in the face of both normal ageing and a number of brain
...
traumatic brain injury.[@psychologicalcorporation2001_WTARWechslerTest] Only in 
Alzheimer's disease were scores significantly lower than in matched
controls.[@psychologicalcorporation2001_WTARWechslerTest; @strauss2006_CompendiumNeuropsychologicalTests]
Those results were, however, published in 2001, well before the formal MDS 
...
promulgated. It is therefore unknown to what extent the findings from that 
original sample remain valid in the face of the significant cognitive 
deterioration that often occurs in Parkinson's. That Parkinson's sample also 
comprised only 10 participants: given the marked heterogeneity of function in 
this disorder, that greatly limits the validity of generalising from those 
findings. In the only study we know of that used the WTAR as a criterion to 
establish mild cognitive impairment in Parkinson's, the proportion classified as
PD-MCI was strikingly higher (79%) than when performance was judged by the more 
conventional method, against norms of current performance (33%). This leads to 
natural questions about whether the WTAR is a valid measure to be used for
cognitive classification in Parkinson's
...
could be examined. To do this, we used latent class trajectory 
analysis[@proust-lima2017estimation-of-e]. This allows for detecting 
heterogeneous patterns of change over time, rather than needing to assign 
individuals to _a priori_ sub-groups. That is, we combined both Control and 
Parkinson's participants in a single pool. The modelling process examines 
individual subjects and places them into clusters depending on similarities in
their longitudinal trajectories. If the presence of either Parkinson's or 
cognitive impairment affects longitudinal WTAR scores, this would be reflected 
by differential membership across groups within the data-driven trajectory 
clusters. As performance on the WTAR is known to be impacted negatively by 
Alzheimer's dementia, and the initial claim of stability in Parkinson's was 
based on very limited evidence[@psychologicalcorporation2001_WTARWechslerTest],
we saw a need to examine its performance in a large and 
...
The New Zealand Parkinson's Progression Programme (NZP<sup>3</sup>) 
[***TODO:*** insert citation] is a
...
the New Zealand Brain Research Institute (NZBRI). Initial, and periodically 
updated, ethical approval was granted by the Health and Disability Ethics 
Committees of the New Zealand Ministry of Health. The study commenced in 2007, 
...
study (see Supplementary Material). All participants were classified as having 
normal cognition, mild cognitive impairment, or dementia via a comprehensive 
...
least one cognitive domain. For PD dementia, a significant impairment 
($\geq$ 2.0 standard deviations below normative data) in at least two cognitive 
domains was required, as well as evidence of significant impairment in everyday
...
identified as Pākehā and `r n_control_other` was 'Other'. All participants were 
speakers of New Zealand English, and were assessed against the US norms of the 
WTAR, corrected for age, sex, and years of education (the provided ethnicity 
...
for data manipulation and visualisation. The dependent variable was 
WTAR-estimated premorbid IQ, initially modelled within each subject as a 
...
informed by any other variables (such as diagnostic categories of MCI or 
dementia).

A latent class model is fitted by first specifying an _a priori_ number of 
classes (i.e. clusters) into which the participants can be divided, and then it
determines what trajectory best describes the longitudinal 
change in scores over time in each of those classes. For example, if just one 
cluster is specified, then the trajectory that is produced will describe the 
average change in score over time for all participants. If two classes are 
specified, then the model will determine the two trajectories that optimally 
separate the total sample into two clusters on the basis of differing 
performance over time. Multiple independent models are fitted, with increasing 
numbers of classes. Those models are then compared to see which produces the 
best description of the data. The classes are termed 'latent' as they are not 
known _a priori_, but instead arise from the data. The resulting classes can,
however, then be compared to known groupings (such as Parkinson's vs Controls).
This allows us to examine to what extent performance over time might be driven
by known factors. For example, if Control and Parkinson's participants perform
differently over time, they should fall unequally into the various latent 
classes (for models with more than one class).

We fitted five separate models, with the specified number of latent classes 
ranging from 1 to 5. The _gridsearch()_ function of the _lcmm_ package was 
used to run 500 departures from random initial values for each multi-class 
model, using the one-class model from which to generate the starting values. The
maximum number of iterations within the _hlme()_ function was set at 1000. 
...
allowed us to detrmine whether longitudinal performance on the WTAR was best 
described by the particpants falling into either 1, 2, 3,4, or 5 underlying
classes. 

When reporting values from the chosen model, we formed the interval in brackets 
following the maximum likelihood estimate (MLE) by subtracting and adding 
1.96 times the standard error given by _hlme_ to the MLE.
...
of each group's latest score is given in Table \@ref(tab:table-1), with
...
participants whose overall cognitive performance declined substantially, with 
progression to dementia (Figure \@ref(fig:figure-2-trajectories)B). We modelled 
...
\@ref(fig:figure-2-trajectories)A. We labelled the first trajectory 'Typical', 
...
The second trajectory we labelled 'High performers'. This class, capturing the 
remaining 
`r with(class_proportions, proportion[class == 'High performers'])` 
of all participants, had an intercept of 
`r high_intcpt_str` `r high_intcpt_intrvl` and a slope of 
`r high_slope_str` `r high_slope_intrvl` points per decade (effectively flat).
That is, both trajectories were relatively stable over time, and were 
...
distribution was skewed well above the population norm mean of 100. Figure 
available under an open CC-BY licence ***NB need to upload this modified version
to FigShare.*** [@macaskill2022_DistributionWechslerTest]
...
figure_1_mod = dat %>% 
  ...
  facet_wrap(~ cognitive_group, ncol = 1) +
...
Unlike in Alzheimer's disease, we found that WTAR scores were relatively stable 
over time in Parkinson's, despite many of our participants undergoing 
substantial declines in overall cognitive performance over the course of the 
study (Figure \@ref(fig:figure-2-trajectories)B). Latent class analysis revealed
that in our particular sample, people fell into one of two trajectory classes, 
but these appeared to be largely driven by two clusters of performance at
baseline, rather than being trajectories that substantially differed 
...
scores have been associated with better current neuropsychological test 
performance in people with Parkinson's,[@koerts2013_InfluenceCognitiveReserve] 
indicating that higher cognitive reserve may be protective against subsequent 
cognitive decline. This can lead to circular reasoning if the premorbid measure 
is not reliable, because if the disease process also reduces current performance
on that measure, then inferring poorer cognitive reserve would not be valid.
That is, due to current impairments, we would be estimating a lower level of 
...
premorbid IQ scores from the 
WAIS-III[@psychologicalcorporation2001_WTARWechslerTest] (Wechsler Adult 
Intelligence Scale, released in 1997). The WAIS-IV (2008) is now in widespread 
use, and the WAIS-V is currently under development. The TOPF (Test of Premorbid
Function) estimates WAIS-IV IQ and was designed as a successor to the WTAR, but 
has had limited uptake in the Parkinson's 
field[@bright2020_ComparisonMethodsEstimating] This perhaps reflects the inertia 
of influential studies (including our own), which have a natural desire to 
retain backwards compatibility with measures that they have used over a long 
period of time. The age of an IQ-related test is important. The Flynn 
...
should, however, apply more to the controls than to the Parkinson's group, 
...
of the Parkinson's population at large.
...
WTAR was used as an individualised premorbid reference for those scores, rather 
than a per-test z-score norm of zero, we would expect to see inflated numbers of
participants being classified as MCI and PDD, because of the over-estimation of
premorbid function. The PD-MCI guidelines do not actually formalise how the 
...
their sample of 139 non-demented people with Parkinson's, the WTAR method 
classified many more of them as having PD-MCI (79%) than did a norms-based Level
II classification method (33%). Marras _et al._ interpreted this as evidence 
that most people with Parkinson's have undergone at least some decline relative
to their premorbid function. Conversely it might support our contention that the
WTAR simply over-estimates that premorbid level of function – just as in our 
sample, where the distribution of estimated IQ values was well above the 
...
Parkinson's.[@statucka2021_MulticulturalismChallengeCognitive] This 
...
therefore not surprising that an inherently language-based test faces issues 
with cross-cultural applicability. In subsequent revisions of the PD-MCI 
diagnostic criteria, if premorbid measures continue to be advocated, more 
...
In summary, the WTAR is a measure that is stable in Parkinson's, even in 
the presence of a marked decline in current cognitive performance. This
makes it a valid research tool to probe cognitive reserve and premorbid function
differences across groups. In absolute terms, however, it appears to 
overestimate premorbid IQ, making it unsuitable to use as a benchmark to 
establish a decline in function within individuals. All such measures likely 
suffer from cross-cultural word choice issues even across different varieties of
English. Analogous tests are not able to be created in languages that do not 
share the orthographic peculiarities of English. Therefore, in future revisions 
of the international MDS guidelines, we believe that a test of premorbid 
function like the WTAR should no longer be recommended as one of the means of 
formally establishing cognitive decline at an individual level in Parkinson's. 
Due to its remarkable stability in Parkinson's despite substantial current 
cognitive impairment, this and related measures will remain a valuable research 
tool in the Anglosphere for testing groups for differences on premorbid 
functioning. The absolute estimation of premorbid function still needs to be 
determined in Parkinson's for newer tests such as the TOPF and its successors.
