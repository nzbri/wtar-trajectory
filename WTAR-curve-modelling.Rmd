---
title: "Wechsler Test of Adult Reading in Parkinson’s: a stable yet problematic premorbid measure"
author:
...
keywords: Parkinson disease, cognitive impairment, dementia, neuropsychology, 
  premorbid function
wordcount: 'Abstract: 200 (of 250), main body: 2608 (of 3700)'
editor_options: 
...
function is appealing. The Wechsler Test of Adult Reading
(WTAR)[@psychologicalcorporation2001_WTARWechslerTest] was proposed in the 
guidelines as one of two recommended [***TODO:*** check their exact phrasing.]
tests for this purpose (along with the National Adult Reading Test, 
...
available under an open CC-BY licence.[@macaskill2022_DistributionWechslerTest]
...
(ref:fig-1-mod-caption) The distribution of WTAR-estimated IQ scores across all 
participants, containing at least two observations from each. Observations are 
colour-coded by their cognitive status at each session. Regardless of current 
cognitive status, WTAR scores span the full range. For each group, the 
distribution was skewed well above the population norm mean of 100. Figure 
...
```{r figure-1-modified-histogram, fig.align='center', fig.cap='(ref:fig-1-mod-caption)', fig.height=fig_1_mod_height/2.54, fig.width=fig_1_width/2.54, warning=FALSE}
figure_1_mod = dat %>% 
  ggplot(aes(x = WTAR, fill = cognitive_status)) +
...
ggsave('images/wtar-figure-1-mod-histogram.pdf', 
       width = fig_1_width, height = fig_1_mod_height, units = 'cm')
ggsave('images/wtar-figure-1-mod-histogram.png', 
       width = fig_1_width, height = fig_1_mod_height, units = 'cm', dpi = 600)
...
period of time. The age of an IQ-related test is important. The Flynn effect[@flynn1999_SearchingJusticeDiscovery] is the robust finding that cohort 
...
In summary, the WTAR is a measure that is quite stable in Parkinson's, even in 
the presence of a substantial decline in current cognitive performance. This
...
function within individuals. All such measures likely suffer from cross-cultural
word choice issues even across different varieties of English. Analogous tests 
are not able to be created in languages that do not share the orthographic 
peculiarities of English. Therefore, in future revisions of the international 
MDS guidelines, we therefore believe that a test of premorbid function like the 
WTAR should no longer be recommended as one of the means of formally 
establishing cognitive decline at an individual level in Parkinson's. Due to its
remarkable stability in Parkinson's despite substantial current cognitive 
impairment, this and related measures will remain a valuable research 
